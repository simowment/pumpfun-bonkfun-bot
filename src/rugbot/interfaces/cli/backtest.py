"""rug_backtest CLI — TP×SL grid optimizer (EX-15)."""
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="rug_backtest",
        description="Replay creator/entity launches avenue-by-avenue and optimize TP×SL grid.",
    )
    ...
    p.add_argument("--optimize", action="store_true", help="run TP×SL grid search")
    ...
    p.add_argument("--json", action="store_true", help="machine JSON")
    return p
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    samples = resolve_target_samples(args.target, entity=bool(args.entity))
    report = run_creator_tp_sl_grid_search(
        samples, config, target=args.target, mode="entity" if args.entity else "wallet"
    )
