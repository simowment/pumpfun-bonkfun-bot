...
def main(argv: Sequence[str] | None = None) -> int:  # noqa: C901, PLR0911, PLR0912
    ...
    # paper-only scalper sub-command: `rugbot.interfaces.cli.watch scalper --since 24h --json`
    # (paper only, no orders, validates expectancy before any live authorization)
    import sys as _sys

    _argv = argv if argv is not None else _sys.argv[1:]
    if len(_argv) > 0 and _argv[0] == "scalper":
        from rugbot.interfaces.cli.scalper import main as scalper_main

        return scalper_main(list(_argv[1:]))
    if len(_argv) > 0 and _argv[0] == "config":
        from rugbot.interfaces.cli.config import main as config_main

        return config_main(list(_argv[1:]))
    ...
    if args.intelligence:
        intelligence_wallets = (args.wallet,) if args.wallet else ()
    ...
    try:
        state_dir_early = resolve_state_dir(args.state_dir)
        config = load_sniper_config_db(state_dir_early)
...
async def _run_stream_watch(  # noqa: PLR0913
    *,
    ...
    source = PumpCreateStreamSource(
        ...
        transport=transport,
        pumpportal_api_key=load_provider_settings().pumpportal_api_key,
        processed_handler=(
