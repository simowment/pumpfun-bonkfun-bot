...
class CompactHeader(Widget):
    ...
    def _render_header_line(self) -> str:
        ...
        stream_badges = {
            "pumpportal_live": "[bold green]TRACK PORTAL[/bold green]",
            "wss_live": "[bold green]TRACK WSS[/bold green]",
            "http_catchup": "[bold yellow]TRACK HTTP[/bold yellow]",
            "disconnected": "[bold red]TRACK OFFLINE[/bold red]",
            "stopped": "[dim]TRACK STOPPED[/dim]",
        }
        parts.append(
            stream_badges.get(
                self.stream_status,
                "[bold yellow]TRACK CONNECTING[/bold yellow]",
            )
        )
    ...
    def watch_stream_status(self, _val: str) -> None:
        self._update_display()
