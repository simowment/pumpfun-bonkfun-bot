...
<section>
  ...
  <div class="body">
    ...
    <div class="tags"><span>{wallet.role || 'NO WALLET SELECTED'}</span></div>
    <div class="balance"><small>FINALIZED SOL BALANCE</small><strong>{wallet.balance || '—'}</strong></div>
...
<style>
  ...
  .balance { display: flex; align-items: baseline; justify-content: space-between; padding: 7px; background: var(--surface-container-low); border: 1px solid var(--outline-variant); }
  .balance small { color: var(--on-surface-variant); font: 800 9px var(--font-meta); }
  .balance strong { color: var(--green-ok); font: 800 14px var(--font-data); }
