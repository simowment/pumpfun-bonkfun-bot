...
class PumpCreateV2LaunchSignalAdapterTests(unittest.TestCase):
    ...
    def test_golden_fixture_emits_no_first_buyer_fallback(self) -> None:
        """The real finalized fixture keeps missing first buyer evidence absent."""

        launch = _golden_launch_created_v2()

        result = pump_create_v2_launch_address_signals(
            launch=launch,
            as_of_slot=Slot(430584458),
        )
        ...
        self.assertNotIn(AddressRole.FIRST_BUYER, tuple(item.role for item in signals))
        self.assertEqual(signals[0].launch_id, launch.launch_id)
        self.assertEqual(
            signals[0].evidence_ids,
            (
                f"launch:{launch.launch_id}:slot:430584458:"
                "create_v2:args.creator:"
                "CvoPbuS2AghzVBYJx7HfQGhALiqif4YwWgHvXmhehuJZ",
            ),
        )
        self.assertEqual(
            ...
            (
                f"launch:{launch.launch_id}:slot:430584458:"
                "create_v2:accounts.user:"
                "CvoPbuS2AghzVBYJx7HfQGhALiqif4YwWgHvXmhehuJZ",
            ),
        )
        self.assertEqual(
            signals[2].evidence_ids,
            (
...
def _golden_launch_created_v2() -> LaunchCreatedV2:
    ...
    if isinstance(launch, AbstainResult):
        raise TypeError(launch.message)
    return launch
