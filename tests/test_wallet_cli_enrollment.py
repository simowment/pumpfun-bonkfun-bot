...
def test_enroll_force_allows_enrollment(monkeypatch, tmp_path, capsys) -> None:
    """With --force, a target wallet can be enrolled even if intelligence abstains."""
    creator = "7SV5ocBq8EkKWsHH2ubB7yVAoTTdytt6FUVTeWw2GEbd"
    database_path = tmp_path / "tracker.db"
    monkeypatch.setenv("RUGBOT_DB_PATH", str(database_path))
    monkeypatch.setattr(
    ...
    assert wallet_cli.main(["mint", "--enroll", "--force", "--json"]) == 0
    output = json.loads(capsys.readouterr().out)
    assert output["enrolled"] is True

    repository = SQLiteTrackerRepository(DatabaseManager(database_path))
    assert repository.get_funder(creator) is not None
